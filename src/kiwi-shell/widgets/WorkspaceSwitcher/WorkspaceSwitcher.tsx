...
// The compositor draws the miniatures for as long as it is told to. Opening
// again with the same windows lays nothing out anew, so they are sent from
...
export default function WorkspaceSwitcher({ gdkmonitor }: { gdkmonitor: Gdk.Monitor }) {
    ...
    const menu = () => {
        ...
        centered.set_center_widget(holder)
        return centered
    }
...
// With kiwi-previews in the compositor the windows are drawn over the
// miniatures (services/previews.ts), each cut off at the edge of its card: a
// window that hangs off its workspace hangs off the card too. They are
...
const live = new LiveTiles({
    ...
    radius: 4,
    clipOf: tile => {
        let canvas: Gtk.Widget | null = tile.get_parent()
        while (canvas && !canvas.has_css_class("ws-canvas")) canvas = canvas.get_parent()
        return canvas
    },
})
